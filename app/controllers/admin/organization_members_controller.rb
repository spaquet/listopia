    ...
    if @member.update(role: params[:role])
      respond_to do |format|
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.replace("member_#{@member.id}",
            partial: "member_row", locals: { member: @member, organization: @organization })
        end
      end
    else
      respond_to do |format|
  ...
  def remove
    ...
    if @member.destroy
      respond_to do |format|
        ...
        format.turbo_stream do
          render turbo_stream: turbo_stream.remove("member_#{@member.id}")
        end
      end
    else
      respond_to do |format|
        format.html { redirect_to admin_organization_members_path(@organization), alert: "Unable to remove member." }
        format.turbo_stream { head :unprocessable_entity }
      end
    end
  end
