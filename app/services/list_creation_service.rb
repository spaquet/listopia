...
class ListCreationService
  ...
  def generate_smart_planning_items(title, description, context)
    # Use AI to generate contextually appropriate planning items
    ai_generator = PlanningItemGenerator.new(title, description, context)
    ai_generator.generate_items
  rescue => e
    Rails.logger.warn "AI planning generation failed: #{e.message}, falling back to generic items"
    generate_fallback_items(title, context)
  end

  def generate_fallback_items(title, context)
    # Generic fallback items that work for any planning scenario
    [
      {
