...
class Connectors::Stub::OauthService < Connectors::OauthService
  ...
  def refresh_token!
    return failure(errors: [ "No connector account" ], message: "Token refresh failed") unless connector_account
    return failure(errors: [ "No refresh token" ], message: "Token refresh failed") unless connector_account.refresh_token.present?
