...
class ListHierarchyService < ApplicationService
  ...
  def create_sublist(structure)
    title = structure.is_a?(Hash) ? structure["title"] : structure.to_s
    items_data = structure.is_a?(Hash) ? (structure["items"] || []) : []
    ...
    sublist = List.new(
      ...
      parent_list: @parent_list,
      title: title,
      description: structure.is_a?(Hash) ? structure["description"] : nil,
