...
RubyLLM.configure do |config|
  # In test environment, skip model resolution entirely
  if Rails.env.test?
    # Don't try to resolve models in test environment
...
if defined?(RubyLLM)
  if Rails.env.test?
    Rails.logger.info "✅ RubyLLM configured for TEST (model validation skipped)"
  else
