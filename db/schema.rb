...
ActiveRecord::Schema[8.0].define(version: 2025_09_10_233319) do
  ...
  create_table "conversation_contexts", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.check_constraint "action::text = ANY (ARRAY['list_viewed'::character varying::text, 'list_created'::character varying::text, 'list_updated'::character varying::text, 'list_deleted'::character varying::text, 'list_status_changed'::character varying::text, 'list_visibility_changed'::character varying::text, 'list_duplicated'::character varying::text, 'list_share_viewed'::character varying::text, 'list_ai_context_requested'::character varying::text, 'item_added'::character varying::text, 'item_updated'::character varying::text, 'item_completed'::character varying::text, 'item_deleted'::character varying::text, 'item_assigned'::character varying::text, 'item_uncompleted'::character varying::text, 'collaboration_added'::character varying::text, 'collaboration_removed'::character varying::text, 'chat_started'::character varying::text, 'chat_switched'::character varying::text, 'chat_message_sent'::character varying::text, 'chat_error'::character varying::text, 'page_visited'::character varying::text, 'dashboard_viewed'::character varying::text, 'lists_index_viewed'::character varying::text])", name: "valid_actions"
    t.check_constraint "entity_type::text = ANY (ARRAY['List'::character varying::text, 'ListItem'::character varying::text, 'User'::character varying::text, 'Chat'::character varying::text, 'Page'::character varying::text])", name: "valid_entity_types"
  end
